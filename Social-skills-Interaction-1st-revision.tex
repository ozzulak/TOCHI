...
\begin{itemize}
	...
	\end{itemize}
	\item {\bf Scaffolding beyond the classroom -- mixed learning environment}
	\begin{itemize}
		...
		\item However, these are often hard to learn from directly for students, as either additional facilitation might be needed for the skills to be practiced properly, which is available in class (e.g., \todo{ add example? such as using teacher to prompt the student to do the Turtle etc?}), or \todo{ANOTHER EXAMPLE regarding the post-hoc reflection and learning phase support}? 
		\item The first the need to provide prompts and reminders about the required activities (cf the use of posters to create awareness [ref]), helping recall and eventually automatise proper behaviours
		\item The second then leads to post-hoc analysis of own activity, where technology could provide further data to ground reflection. Draws on the importance to be able to verbalise and analyse what happened, take multiple perspectives. 
		\item 
	\end{itemize}
\end{itemize}
