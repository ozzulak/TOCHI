...
\todo{Key message in this section:} 

\todo{Given we saw how SEL is taught, and the three example areas suggesting it is likely HCI can help... THIS IS WHAT HCI SHOULD DO AS THE NEXT STEPS}

\medskip 

Main points: 
\begin{enumerate}
	\item {\bf Research around SSP and AC work }-- in-out-learning dimension + SEL needs as great opportunity to inspire, guide and test the algorithms
	\begin{itemize}
		\item Test-bed for SSP/AC ... curricula providing a continuum of control-wild settings to test in; as well as structure (specific tasks/skills to be learned and supported) as well as good evaluation methods for testing these. 
		\item As such, [inspire/guide/test] crap. 
		\item Automatic capture and processing of data that is robust and reliable is often difficult in the wild, especially if it is to be meshed fluidly with other complex interactions (as will be the case in both in-session and out of session systems here). However, sensing and processing of aspects relevant to social skills is likely to be challenging by itself, even in controlled laboratory conditions.
...
There is also a question of the level of interpretation we expect the system to provide. This leads to a continuum from leaving the sense-making of raw data entirely to the user and/or the facilitator (e.g., as per SenseCam systems \cite{Fleck2009}), to providing full interpretation by the system (e.g., as in arousal detection for people with autism \cite{Picard2009}). Even if particular concepts cannot be reliably and fully interpreted by technology, it might still be possible, and in many cases actually preferable, to support the users by providing a 'reasonably' pre-processed data they can view, interpret and reflect on. 
	...
	\item {\bf Research around supporting Social reflection}
	\begin{itemize}
		\item steal the ideas (and perhaps the whole subsection?) from CSCW paper ... 
	\end{itemize}
	\item {\bf Participatory approaches to understand the needs of students/parents/teachers}
	\item {\bf Research around SEL support for teachers + parents}-- community building and school-home interaction .. \todo{can I find more examples of such work?}
	\begin{itemize}
		...
		\item Area ripe for inclusion of technology and cultural probes (and similar approaches) to explore the settings and possible use of technology? 
	\end{itemize}
\end{enumerate}
\bigskip
