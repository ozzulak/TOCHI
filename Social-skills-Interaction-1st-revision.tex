...
\todo{1 sentence to say that, for example, aidentifying teachable moments around self-control (before it is too late) is an important part of the learning. }
...
\label{sec:feedback}
%{\bf Key challenge for HCI}: Reflection currently focusses on individual behaviour, initial work supporting emotional tracking, but at the minute mostly everyday pattern tracking (QS). Focus on social aspects is rare, and would pose interesting challenges. How do we develop technology to help children learn reflection processes -- designing for surprise, for off-balance moments, empower exploration of own and others' experience (draw on Moon)?

\begin{itemize}
	\item  The ability to reflect on own and others emotions, thoughts and behaviour is crucial for experiential learning \cite{Moon}, and as such underplays all skills taught in SEL \cite{Cohen2001,others}.
	\item For example, such reflection processes play a key role in identifying and understanding emotion as taught in all curricula, using various tools to support the learners in doing so ( e.g., the Feeling Faces cards) and the exercises aimed to teach the learners to label their emotions. \todo{Similarly, reflective processes play a key role in self-control (being aware one is getting angry), perspective taking ... }.
	\item While existing SEL learning processes are successful to the extent that students \rephrase{learn}, we argue that digital technology has the potential to further extend and augment such training. \midskip
	\item Surprise and novel viewpoints on data are known to elicit and facilitate reflection \cite{Moon,Fleck}, which is why providing the learners with cues to elicit reflection is important.
\item Prior HCI research suggests the potential digital technology has  provide such novel cues and thus scaffold reflective processes, often through drawing out data and patterns that would be otherwise inaccessible. 
	\item In part, this is due to the recent interest in HCI on using wearable sensors and other devices (e.g., mobiles phones) to track aspects that could be particularly relevant for reflecting on social and emotional skills, such as emotion detection and social signals processing.  
	...
	\item We outline three relevant topics where initial HCI work exists: emotional awareness; mindfulness and calming technology; and cues around communication skills. 
	
\end{itemize}


% {{{ Emotional awareness
        \subsubsection{Emotional awareness}~\\
        \label{sec:emaware}
Developing emotional awareness is the founding part of every SEL curricula.   Much work has been done recently in HCI around enhancing supporting similar reflection processes (e.g., \cite{Fleck2010,Sas2011}). 
...
%Moreover, the the well-motivated and defined problems faced in social and emotional learning are likely to inspire novel systems and technologies addressing these complex topics \rephrase{...and blah blah :)}
