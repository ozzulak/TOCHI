...
The review of SEL in schools have clearly outlined the fundamental role of experiential learning in development of social and emotional skills. We now highlight embedding as the key aspect through which digital technology may address a crucial need within SEL learning. Moreover, we also highlight two other areas where, based on existing work in HCI, we argue use of digital technology could bring novel opportunities to support and enhance current learning. These are in particular support for promoting reflection and awareness of own and others' emotional states, and novel opportunities for practice. 


\paragraph{Transitioning the skills out of class into everyday situations}
...
\begin{itemize}
	...
	\begin{itemize}
		\item extending the learning support and scaffolding for learners beyond the lessons themselves, e.g., through mobile and sensor based technology.
		\item supporting creation of a wider community around learning of social skills, including involvement of parents, teachers, and peers. 
	\end{itemize}
	\item {\bf Focus on learners -- support beyond the SEL specific activities}
	\begin{itemize}
		\item When SEL skills are to be learned/transferred beyond the SEL related activities, the learners can no longer take the advantage of: 
		\begin{itemize}
			\item \emph{Scaffolding and structure} -- lack of the structure and scaffolding by the teacher present in the session
			... 
		  \item \emph{Facilitate teachable moments detection} learning activities are intertwined with other interactions -- it is difficult to identify the teachable moments (although many are available)
			\item \emph{Support opportunities for reflection} students might not be supported to step away and reflect on what was happening as other aspects are in play
		\end{itemize}
		\item we envision technology could support each of these: 
		\begin{itemize}
			\item Real-time prompts and reminders to support the scaffolding of activities -- e.g., MOSOCO for autism, as PopTherapy as a stress relief; experience sampling/SenseCam to promote awareness of own states, plans and time.   
			\item Possibilities for tracking and suggestions for teachable moments -- e.g., strong emotion detection through physiological data (); or audio-recognition algorithms ... also something utilising the social dynamics and observers. 
		\end{itemize}
...
\end{itemize}
	\item {\bf Facilitating supportive environment}
	\begin{itemize}
		\item parents
		\item teachers 
...
\begin{itemize}
	...
	\item {\bf age appropriate -- use the table from Elias to give examples with a reference}
